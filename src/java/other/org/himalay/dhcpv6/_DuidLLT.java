...
public class _DuidLLT extends DuidFactory.Duid { // Concrete type is DuidLLT
	...
	public _DuidLLT() // throws Exception
	{
		init();
	}
	...
	public int readNoHeader(DataInputStream istream) throws IOException {
		...
		// read time
		{
			time = (long) (BinPrimitive.readUI32(istream));
			retVal += 4;
		}
		// read llAddress
	...
	public int write(DataOutputStream ostream) throws IOException {
		...
		retVal += 2;
		// write time
		BinPrimitive.writeUI32(ostream, time);
		retVal += 4;
		// write llAddress
		if (llAddress != null)
	...
	public int dump(DumpContext dc) throws IOException {
		...
		dc.indent();
		dc.getPs().println(
				"time=" + time + "(0x" + Long.toHexString(time) + ")");
		// write llAddress
		if (llAddress != null) {
			dc.indent();
